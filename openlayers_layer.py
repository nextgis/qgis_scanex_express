...
class OpenlayersLayer(QgsPluginLayer):
  ...
  def readXml(self, node):
    # custom properties
    self.layerType = "ScanexExpress"
    self.layerName = node.toElement().attribute("layer_name", "")
    self.layerCrs = node.toElement().attribute("layer_crs", "")
    self.apiKey = node.toElement().attribute("api_key", "")
    return True
  ...
  def writeXml(self, node, doc):
    ...
    # custom properties
    element.setAttribute("layer_name", str(self.layerName))
    element.setAttribute("layer_crs", str(self.layerCrs))
    element.setAttribute("api_key", str(self.apiKey))
    return True
